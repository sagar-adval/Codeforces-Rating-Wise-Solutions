...
int32_t main() {
    ...
    while (t--) {
        ...
        f(i, 0, l - 1)
        {
            ...
            {
                if (a[v[i].first.second - 1] < a[v[i + 1].first.first] || a[v[i].first.second] < a[v[i + 1].first.first + 1]) {
                    ans = max(ans, v[i].second + v[i + 1].second - 1);
                }
            }
        }
        ans1 = v[0].second;
        f(i, 0, l)
        {
            if (v[i].second > ans1) ans1 = v[i].second;
        }
